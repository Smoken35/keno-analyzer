...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            os.path.join(os.path.dirname(os.path.dirname(__file__)), "logs", "service_manager.log")
        ),
    ],
)
...
def save_pids(pids):
    ...
    with open(PIDS_FILE, "w") as f:
        json.dump({"pids": pids, "timestamp": datetime.now().isoformat()}, f)
            ...
            try:
                for conn in proc.connections():
                    if conn.laddr.port == port:
                        logger.warning(
                            f"Port {port} is in use by process {proc.pid} ({proc.name()})"
                        )
                        return True
        ...
        for proc in psutil.process_iter(["pid", "name", "connections"]):
            try:
                for conn in proc.connections():
                    if conn.laddr.port == port:
                        logger.info(
                            f"Terminating process {proc.pid} ({proc.name()}) on port {port}"
                        )
            ...
            while retries > 0:
                try:
                    ...
                    if process.poll() is not None:
                        ...
                        logger.error(
                            f"{service_name} process exited with code {process.returncode}"
                        )
    ...
    try:
        ...
        with open(
            ...
            metrics_proc = subprocess.Popen(
                [sys.executable, metrics_script], stdout=log, stderr=log, preexec_fn=os.setpgrp
            )
        ...
        with open(
            ...
            alert_proc = subprocess.Popen(
                [sys.executable, alert_script], stdout=log, stderr=log, preexec_fn=os.setpgrp
            )
        ...
        metrics_ok = verify_service(
            "Metrics Generator", 8000, "http://localhost:8000/metrics", timeout=30
        )
        alert_ok = verify_service(
            "Alert Receiver", 3456, "http://localhost:3456/health", timeout=30
        )
