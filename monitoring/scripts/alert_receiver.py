...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'logs', 'alert_receiver.log'))
    ]
)
...
def process_alert(alert):
    ...
    try:
        ...
        if alert.get('labels', {}).get('severity') == 'critical':
            logger.warning(f"Critical alert received: {alert['annotations'].get('description', 'No description')}")
        ...
        while retry_count < max_retries and running:
            ...
            try:
                ...
                if "Address already in use" in str(e):
                    logger.error(f"Port {port} is in use by another program. Either identify and stop that program, or start the server with a different port.")
                port += 1
                retry_count += 1
