    ...
    def post_process_predictions(self, predictions):
        ...
        if len(unique_numbers) < 20:
            ...
            additional = np.random.choice(
                np.setdiff1d(np.arange(1, 81), unique_numbers),
                size=remaining,
                replace=False
            )
        ...
        try:
            ...
            report = {
                ...
                'model_confidence': {
                    ...
                    'neural_network': float(nn_model.evaluate(X_scaled, nn_predictions, verbose=0)[0])
                }
            }
            
            # Save predictions
            output_file = self.models_dir / f"predictions_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
