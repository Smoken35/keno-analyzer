...
from keno import KenoAnalyzer, KenoVisualizer, ValidationTracker
...
def verify_core_functionality():
    ...
    try:
        ...
        numbers = [
            sorted(np.random.choice(range(1, 81), size=20, replace=False)) for _ in range(100)
        ]
...
def check_data_integrity():
    ...
    try:
        ...
        # Generate sample data for testing
        numbers = [
            sorted(np.random.choice(range(1, 81), size=20, replace=False)) for _ in range(30)
        ]
        analyzer.data = numbers
...
def verify_performance_metrics():
    ...
    try:
        ...
        for method in methods:
            ...
            simulation = analyzer.calculate_expected_value(pick_size=4, method=method)
            ...
            p_value = stats.ttest_1samp(historical_results, 2.0)[
                1
            ]  # Test against random expectation

            results[method] = {
                "simulation": {"roi_percent": (simulation - 1.0) * 100, "num_simulations": 100},
                "validation": {"avg_matches": float(avg_matches), "p_value": float(p_value)},
            }
...
def generate_verification_report(results):
    ...
    if not results["core_functionality"]:
        report["recommendations"].append("Review and fix core functionality issues")
    if not results["data_integrity"]:
        report["recommendations"].append("Address data integrity concerns")
    if not results["performance_metrics"]:
        report["recommendations"].append("Investigate performance metrics issues")
