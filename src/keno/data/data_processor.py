    ...
    def process_uploaded_file(self, file_path):
        ...
        try:
            ...
            if isinstance(df['winning_numbers'].iloc[0], str):
                df['winning_numbers'] = df['winning_numbers'].apply(lambda x: json.loads(x.replace("'", '"')))
        ...
        try:
            if os.path.exists(all_time_file) and os.path.getsize(all_time_file) > 0:
                ...
                if isinstance(all_time_df['winning_numbers'].iloc[0], str):
                    all_time_df['winning_numbers'] = all_time_df['winning_numbers'].apply(lambda x: json.loads(x.replace("'", '"')))
