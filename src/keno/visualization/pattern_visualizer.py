...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('keno_data/pattern_visualization.log'),
...
ALERT_COUNTER = Counter(
    'pattern_alert_count',
    'Number of pattern alerts generated',
    ['alert_type']
)
        ...
        try:
            with VISUALIZATION_TIME.labels(visualization_type='pattern_analysis').time():
                ...
                for row in range(8):
                    for col in range(10):
                        num = row * 10 + col + 1
                        plt.text(col + 0.5, row + 0.3, str(num), ha="center", fontsize=8, color="black")
                ...
                try:
                    ...
                    significant_pairs = [pair for pair, data in results["pair_frequency"].items() 
                                      if data["ratio"] > 2.0]
                    ...
                    node_sizes = []
                    for node in G.nodes():
                        freq = results["number_frequency"].get(str(node), {}).get("percentage", 1.25)
                ...
                # Row bias
                row_values = [results["grid_patterns"]["row_bias"][row]["bias_ratio"] for row in range(8)]
                ...
                # Column bias
                col_values = [results["grid_patterns"]["col_bias"][col]["bias_ratio"] for col in range(10)]
                ...
                # Quadrant bias
                quadrant_values = [results["grid_patterns"]["quadrant_bias"][q]["bias_ratio"] for q in range(4)]
                sns.barplot(x=["Q1 (TL)", "Q2 (TR)", "Q3 (BL)", "Q4 (BR)"], y=quadrant_values, ax=ax3)
                ...
                significant_bias = any(abs(v - 1) > 0.2 for v in row_values + col_values + quadrant_values)
                if significant_bias:
                    self.send_alert(
                        severity="warning",
                ...
                top_sequential = sorted(
                    [(int(num), data["follow_probability"]) for num, data in results["sequential_patterns"].items() 
        ...
        try:
            with VISUALIZATION_TIME.labels(visualization_type='walk_forward_analysis').time():
                ...
                for method in results["method_performance"]:
                    indices = [entry["window_index"] for entry in results["method_performance"][method]]
                ...
                if "adaptive_ensemble" in results and "window_results" in results["adaptive_ensemble"]:
                    indices = [entry["window_index"] for entry in results["adaptive_ensemble"]["window_results"]]
                    roi_values = [entry["roi"] for entry in results["adaptive_ensemble"]["window_results"]]
                    
                    plt.plot(indices, roi_values, label="adaptive_ensemble", linewidth=2, linestyle='--')
                ...
                positive_percentage = [results["overall_results"][m]["positive_percentage"] for m in methods]
                ...
                ax2 = ax1.twinx()
                bars2 = ax2.bar(x + width/2, positive_percentage, width, color='orange', label='Positive Windows (%)')
                ...
                # Save plot
                plt.tight_layout()
                method_comparison_path = os.path.join(self.viz_dir, "method_performance_comparison.png")
                ...
                if results["adaptive_weights"]:
                    ...
                    for method in methods:
                        ...
                        weights = [entry["weights"][method] for entry in results["adaptive_weights"]]
