...
class KenoVisualizer:
    ...
    def plot_pattern_analysis(self, output_path: str):
        ...
        ax1.bar(
            x - width / 2,
            [frequencies[n] for n in patterns["hot_numbers"]],
            width,
            label="Hot Numbers",
        )
        ...
        ax2.bar(
            x - width / 2,
            [frequencies[n] for n in patterns["cold_numbers"]],
            width,
            label="Cold Numbers",
        )
    ...
    def plot_validation_results(self, validation_tracker, filename: str) -> None:
        """
        ...
        for method in validation_tracker.history["method"].unique():
            method_data = validation_tracker.history[validation_tracker.history["method"] == method]
            ax1.plot(method_data.index, method_data["accuracy"] * 100, label=method, marker="o")
        ...
        sns.boxplot(data=validation_tracker.history, x="method", y="accuracy", ax=ax2)
    ...
    def plot_expected_value_analysis(self, payout_table: str, filename: str) -> None:
        """
        ...
        for pick_size in pick_sizes:
            for method in methods:
                ev = self.analyzer.calculate_expected_value(pick_size, payout_table, method)
                results.append({"pick_size": pick_size, "method": method, "expected_value": ev})

        df = pd.DataFrame(results)

        plt.figure(figsize=(12, 8))
        sns.lineplot(data=df, x="pick_size", y="expected_value", hue="method", marker="o")
    ...
    def create_dashboard(self, validation_tracker) -> None:
        ...
        self.plot_prediction_comparison(
            4, 100, os.path.join(self.save_path, "prediction_comparison.png")
        )
        ...
        if self.analyzer.payout_tables:
            self.plot_expected_value_analysis(
                list(self.analyzer.payout_tables.keys())[0], "expected_value_analysis.png"
            )
