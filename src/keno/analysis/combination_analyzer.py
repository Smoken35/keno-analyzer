...
class CombinationAnalyzer:
    ...
    def analyze_number_relationships(self) -> Dict[str, Any]:
        ...
        try:
            relationships = defaultdict(lambda: {'appears_with': defaultdict(int), 'total_appearances': 0})
    ...
    def find_winning_patterns(self, min_size: int = 3, max_size: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return sorted(patterns, 
                        key=lambda x: (x['frequency'] * 0.4 + 
                                     x['recent_frequency'] * 0.4 + 
                                     x['trend'] * 0.2), 
                        reverse=True)
    ...
    def plot_relationship_heatmap(self):
        ...
        try:
            ...
            # Create matrix
            numbers = sorted(list(set([r['number1'] for r in relationships] + [r['number2'] for r in relationships])))
    ...
    def print_analysis(self, combination_size: int = 3):
        ...
        for size in [3, 4, 5]:
            ...
            if combos:
                best = combos[0]
                print(f"   Size {size}: {best['combination']} (Frequency: {best['frequency']:.1f}%)")
