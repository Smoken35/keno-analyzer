...
class PayoutAnalyzer:
    ...
    def calculate_expected_value(self, variant_name: str, pick_size: int, bet_amount: float = 1.0) -> Optional[Dict]:
        """
    ...
    def _calculate_probability(
        self, total: int, drawn: int, picked: int, matches: int
    ) -> float:
        """
        ...
        try:
            numerator = (
                self._combinations(picked, matches) * 
                self._combinations(total - picked, drawn - matches)
            )
    ...
    def get_best_play_type(self, variant_name: str, bet_amount: float = 1.0) -> Optional[Tuple[int, Dict]]:
        """
