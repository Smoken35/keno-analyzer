...
def handle_analyze_command(args: argparse.Namespace) -> Dict:
    ...
    if args.type in ["all", "strategy"]:
        ...
        if ev_comparison:
            results["strategy"] = {
                "best_play": max(ev_comparison.items(), key=lambda x: x[1]["return_percentage"])[0]
            }
...
def handle_strategy_command(args: argparse.Namespace) -> Dict:
    ...
    comparison = analyzer.compare_advanced_strategies(
        "BCLC", bet_amount=1, num_draws=args.simulations
    )
...
def parse_args() -> argparse.Namespace:
    ...
    data_parser.add_argument(
        "--source", default="sample", help="Data source (sample, bclc, or URL)"
    )
    data_parser.add_argument("--csv", help="Load data from CSV file")
    data_parser.add_argument("--output", help="Output file for scraped data")
    ...
    analyze_parser.add_argument("--source", default="sample", help="Data source")
    analyze_parser.add_argument("--csv", help="Load data from CSV file")
    ...
    predict_parser.add_argument("--picks", type=int, default=4, help="Number of numbers to pick")
    predict_parser.add_argument(
        "--compare", action="store_true", help="Compare all prediction methods"
    )
    predict_parser.add_argument(
        "--simulate", action="store_true", help="Simulate prediction performance"
    )
    predict_parser.add_argument(
        "--draws", type=int, default=100, help="Number of draws to simulate"
    )
    predict_parser.add_argument("--source", default="sample", help="Data source")
    predict_parser.add_argument("--csv", help="Load data from CSV file")
    ...
    viz_parser.add_argument("--source", default="sample", help="Data source")
    viz_parser.add_argument("--csv", help="Load data from CSV file")
    viz_parser.add_argument(
        "--output-dir", default="keno_visualizations", help="Output directory for visualizations"
    )
    viz_parser.add_argument(
        "--dashboard", action="store_true", help="Create comprehensive dashboard"
    )
    viz_parser.add_argument(
        "--dashboard-file", default="keno_dashboard.pdf", help="Filename for dashboard PDF"
    )
    ...
    strategy_parser.add_argument(
        "--picks", type=int, default=4, help="Number of picks to optimize for"
    )
    strategy_parser.add_argument(
        "--simulations", type=int, default=100, help="Number of simulations to run"
    )
    strategy_parser.add_argument("--source", default="sample", help="Data source")
    strategy_parser.add_argument("--csv", help="Load data from CSV file")
    strategy_parser.add_argument("--output", help="Output file for strategy results")
