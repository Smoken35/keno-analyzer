...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("cluster_visualization.log"), logging.StreamHandler()],
)
...
class ClusterVisualizer:
        ...
        for cluster in self.clusters:
            ...
            for i in range(len(numbers)):
                for j in range(i + 1, len(numbers)):
                    G.add_edge(numbers[i], numbers[j], weight=cluster["cohesion"])
    ...
    def plot_metric_distributions(self, output_file: str = "results/metric_distributions.png"):
        ...
        # Prepare data
        metrics = ["quality_score", "cohesion", "coverage", "frequency"]
        data = {metric: [cluster[metric] for cluster in self.clusters] for metric in metrics}
...
def main():
    ...
    parser.add_argument(
        "--results", required=True, help="Path to cluster analysis results JSON file"
    )
    parser.add_argument(
        "--output-dir", default="results", help="Directory to save visualization files"
    )
