...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("randomness_plots.log"), logging.StreamHandler()],
)
...
class KenoRandomnessVisualizer:
    ...
    def plot_entropy_distribution(self):
        ...
        sns.histplot(data=self.results["entropy"]["window_entropies"], bins=30, kde=True)
        ...
        plt.axvline(
            x=max_entropy, color="r", linestyle="--", label=f"Theoretical Max: {max_entropy:.2f}"
        )
    ...
    def plot_temporal_drift(self):
        ...
        sns.heatmap(drift_data, cmap="RdYlBu_r", center=0, xticklabels=True, yticklabels=True)
    ...
    def plot_autocorrelation(self):
        ...
        sns.heatmap(acf_data, cmap="RdYlBu_r", center=0, xticklabels=True, yticklabels=True)
    ...
    def plot_overall_score(self):
        ...
        plt.axhline(
            y=overall_score, color="r", linestyle="--", label=f"Overall Score: {overall_score:.2f}"
        )
...
def main():
    ...
    parser.add_argument(
        "--report", required=True, help="Path to the randomness audit report JSON file"
    )
