...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("cluster_analysis.log"), logging.StreamHandler()],
)
logger = logging.getLogger(__name__)


def load_transactions(db_path: str, start_date: str = None, end_date: str = None) -> List[Set[int]]:
    """
...
def main():
    ...
    parser.add_argument(
        "--min-cohesion", type=float, default=0.3, help="Minimum cluster cohesion threshold"
    )
    parser.add_argument(
        "--min-coverage", type=float, default=0.2, help="Minimum cluster coverage threshold"
    )
    parser.add_argument("--output", default="results/clusters.json", help="Output file path")

    args = parser.parse_args()

    # Load transactions
    logger.info("Loading transactions from database...")
    transactions = load_transactions(args.db, start_date=args.start_date, end_date=args.end_date)
    ...
    cluster_builder = ClusterBuilder(min_cohesion=args.min_cohesion, min_coverage=args.min_coverage)

    clusters = cluster_builder.build_clusters(transactions, n_clusters=args.n_clusters)
