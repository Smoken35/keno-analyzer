...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("keno_stats.log"), logging.StreamHandler()],
)
...
class KenoStatsGenerator:
    ...
    def get_temporal_stats(self, interval: str = "month") -> List[Dict]:
        ...
        return [
            {"period": row[0], "games_count": row[1], "unique_games": row[2]}
    ...
    def get_number_patterns(self) -> Dict:
        ...
        patterns["most_common_pairs"] = [
            {"numbers": (n1, n2), "frequency": freq} for n1, n2, freq in self.cursor.fetchall()
        ]
