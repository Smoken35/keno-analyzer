...
class KenoAnalyzer:
    ...
    def predict_next_draw(self, method: str = "frequency") -> List[int]:
        ...
        if method == "frequency":
            ...
            return sorted(np.random.choice(hot_nums + cold_nums, size=20, replace=False))
        else:
    ...
    def _calculate_hit_probability(self, prediction: List[int], hits: int) -> float:
        """Calculate probability of getting exactly n hits."""
        total_matches = sum(
            1 for draw in self.data[-100:] if len(set(prediction) & set(draw)) == hits
        )
