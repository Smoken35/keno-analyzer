...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("module3_report.log"), logging.StreamHandler()],
)
...
class Module3ReportGenerator:
    ...
    def _generate_summary_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "rules": {
                ...
                "avg_size": np.mean(
                    [len(r["antecedent"]) + len(r["consequent"]) for r in self.results["rules"]]
                ),
            },
            "clusters": {
    ...
    def _generate_pattern_plots(self) -> Dict[str, str]:
        ...
        top_patterns = sorted(self.results["patterns"], key=lambda x: x["support"], reverse=True)[
            :20
        ]
    ...
    def _generate_rule_plots(self) -> Dict[str, str]:
        ...
        plt.figure(figsize=(12, 8))
        G = nx.DiGraph()
        top_rules = sorted(self.results["rules"], key=lambda x: x["lift"], reverse=True)[:20]
    ...
    def _generate_cluster_plots(self) -> Dict[str, str]:
        ...
        scatter = plt.scatter(sizes, qualities, c=cohesions, cmap="viridis", alpha=0.6)
        ...
        plt.figure(figsize=(12, 8))
        top_clusters = sorted(
            self.results["clusters"], key=lambda x: x["quality_score"], reverse=True
        )[:20]
    ...
    def _get_top_items(self, n: int = 5) -> Dict[str, List[Dict]]:
        ...
        top_items = {
            "patterns": sorted(self.results["patterns"], key=lambda x: x["support"], reverse=True)[
                :n
            ],
            "rules": sorted(self.results["rules"], key=lambda x: x["lift"], reverse=True)[:n],
            "clusters": sorted(
                self.results["clusters"], key=lambda x: x["quality_score"], reverse=True
            )[:n],
        }
