...
class ClusterBuilder:
    ...
    def _create_similarity_matrix(self, cooccurrence_matrix: np.ndarray) -> np.ndarray:
        ...
        for i in range(80):
            for j in range(i + 1, 80):
                ...
                union = (cooccurrence_matrix[i][i] + 
                        cooccurrence_matrix[j][j] - 
                        intersection)
    ...
    def _calculate_cluster_metrics(self, 
                                 cluster: Set[int], 
                                 transactions: List[Set[int]]) -> Tuple[float, float, float]:
        """
        ...
        """
        # Calculate frequency
        cluster_count = sum(1 for t in transactions 
                          if any(n in t for n in cluster))
        frequency = cluster_count / self.transaction_count
        
        # Calculate coverage (how often all numbers appear together)
        full_matches = sum(1 for t in transactions 
                         if cluster.issubset(t))
    ...
    def build_clusters(self, 
                      ...
                      n_clusters: int = 10) -> List[NumberCluster]:
        """
        ...
        for cluster_id in range(n_clusters):
            cluster_numbers = [i + 1 for i, label in enumerate(labels) 
                             if label == cluster_id]
            ...
            if cluster_numbers:  # Skip empty clusters
                ...
                if (cohesion >= self.min_cohesion and 
                    coverage >= self.min_coverage):
                    self.clusters.append(NumberCluster(
