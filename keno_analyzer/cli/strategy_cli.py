...
def main():
    ...
    parser.add_argument("--strategy", choices=["pattern", "rule", "cluster"], default="pattern", help="Strategy to use")
    parser.add_argument("--draw-index", type=int, help="Draw index to predict (must be < len(data))")
    parser.add_argument("--num-picks", type=int, default=20, help="Number of numbers to predict (default: 20)")
    ...
    strategy_map = {
        "pattern": "PatternBased",
        "rule": "RuleBased",
        "cluster": "ClusterBased"
    }
    ...
    if args.evaluate:
        ...
        result = engine.evaluate_strategy(strategy_name, data, start_index=args.start_index, end_index=end_index)
        ...
        if args.details:
            ...
            for r in result['detailed_results']:
                print(f"Draw {r['draw_index']} | Hits: {r['hits']} | Confidence: {r['confidence']:.3f}")
