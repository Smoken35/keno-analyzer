...
from keno.data.playnow_scraper import PlayNowScraper


class TestPlayNowScraper(unittest.TestCase):
    """Test cases for the PlayNowScraper class."""

    def setUp(self):
        """Set up test environment."""
        # Create a temporary directory for test data
        self.test_dir = tempfile.mkdtemp()
        self.scraper = PlayNowScraper(output_dir=self.test_dir)
    ...
    def test_convert_to_standard_format(self):
        ...
        # Check content
        df_standardized = pd.read_csv(standardized_path, dtype={"DRAW_NUMBER": str})
    ...
    def test_duplicate_handling(self):
        ...
        consolidated_path = os.path.join(self.test_dir, "csv", "keno_results_all.csv")
        import pandas as pd

        df = pd.read_csv(consolidated_path, dtype={"draw_number": str})
