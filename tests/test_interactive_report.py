...
from keno.scripts.interactive_report import analyze_patterns, calculate_trend, generate_report
...
def sample_results():
    ...
    return {
        "PatternStrategy": {
            ...
            "hit_rate": 0.62,
            "detailed_results": [{"hits": 8, "confidence": 0.7}] * 50
            + [{"hits": 9, "confidence": 0.75}] * 50,
        },
        "RuleStrategy": {
            "total_predictions": 100,
            "average_hits": 7.5,
            ...
            "hit_rate": 0.55,
            "detailed_results": [{"hits": 7, "confidence": 0.6}] * 50
            + [{"hits": 8, "confidence": 0.7}] * 50,
        },
    }
...
def sample_csv_path(tmp_path, sample_results):
    ...
    with open(csv_path, "w") as f:
        ...
        for strategy, data in sample_results.items():
            f.write(
                f"{strategy},{data['total_predictions']},{data['average_hits']},{data['hit_rate']},{data['average_confidence']}\n"
            )
    return str(csv_path)
...
class TestInteractiveReport:
    ...
    def test_calculate_trend(self):
        ...
        assert trend[-1] == pytest.approx(4.67, rel=0.01)  # Last 3 numbers average
        assert trend[0] == pytest.approx(1.0)  # First number
        assert trend[1] == pytest.approx(1.5)  # Average of first 2 numbers
        assert trend[2] == pytest.approx(2.0)  # Average of first 3 numbers
    ...
    def test_generate_interactive_report(
        self, tmp_path, sample_results, sample_plot_paths, sample_csv_path
    ):
    ...
    def test_generate_interactive_report_with_missing_plots(
        self, tmp_path, sample_results, sample_csv_path
    ):
