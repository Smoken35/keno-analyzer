            ...
            results.append(
                {
                    ...
                    "avg_matches": np.mean(
                        [m for m, c in sim_result["match_distribution"].items() for _ in range(c)]
                    ),
                    "max_consecutive_payouts": max(
                        analyze_consecutive_payouts(analyzer, pick_size, num_simulations=100)[
                            method
                        ]
                    ),
                }
...
def main():
    ...
    consecutive_results = analyze_consecutive_payouts(
        analyzer, best_strategy["pick_size"], num_simulations=1000
    )

    # Plot consecutive payouts
    plot_consecutive_payouts(
        consecutive_results, os.path.join(output_dir, "consecutive_payouts.png")
    )
    ...
    prediction = analyzer.predict_next_draw(best_strategy["method"], best_strategy["pick_size"])
